...
router.get("/:name?", verifyToken, async function (req, res, next) {
  if (req.params.name) {
    var sql = `SELECT * FROM sellers WHERE (CONCAT(firstname, ' ', lastname) LIKE ?)`;
    name = "%" + req.params.name + "%";
    await db.query(sql, [name], function (err, result) {
      res.status(201).json({ result: result });
    });
  } else {
    var sql = ` SELECT sellers.* FROM sellers WHERE 1;`;
...
router.delete("/:id", verifyToken, async function (req, res, next) {
  try {
    var sql = `SELECT sellers.avatar FROM sellers WHERE sellers.id = ?`;
    await db.query(sql, [req.params.id], function (err, result) {
      ...
      (async () => {
        try {
          var sql = `DELETE FROM sellers WHERE id = ?`;
          await db.query(sql, [req.params.id], function (err, result) {
            res.status(201).json({ result: result });
          });
        } catch (err) {
